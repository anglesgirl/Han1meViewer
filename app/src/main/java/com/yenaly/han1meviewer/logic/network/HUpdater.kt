...
object HUpdater {
    ...
    private suspend fun File.downloadTo(
        ...
        // ── 本 fork 专属：github.com 的 release 包走国内镜像更稳（镜像优先、直连兜底）──
        // 上游原实现只有一次直连请求；大陆网络下 github release 直连经常断，故逐个候选重试。
        // 非 github.com 的地址（例如 CI 产物的直链）保持单候选，行为不变。
        var res: retrofit2.Response<okhttp3.ResponseBody>? = null
        var lastErr: Exception? = null
        for (candidate in mirrorUrls(url)) {
            try {
                val r = HanimeNetwork.githubService.request(candidate)
                if (r.isSuccessful) {
                    res = r
                    lastErr = null
                    break
                }
                lastErr = IOException("HTTP ${r.code()}: $candidate")
            } catch (e: Exception) {
                lastErr = e
            }
        }
        val response = res ?: throw HUpdaterException.Download(
            message = "请求下载地址失败：${lastErr?.message ?: lastErr?.let { it::class.java.simpleName } ?: "unknown"}",
            cause = lastErr,
            reason = HUpdaterException.Download.Reason.NETWORK,
        )
        // 先确认拿到的是安装包，再决定是否落盘
        val body = response.orThrowDownload(url)
        if (isZip) {
